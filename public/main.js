    ...
    sortedTrades.forEach((t, index) => {
        try {
            ...
            row.innerHTML = `
                ...
                <td class="text-end text-warning" style="vertical-align: middle; font-size: 0.7rem;">${isBuy ? '-' : (t.fees ? '$' + parseFloat(t.fees).toFixed(4) + (t.feeCurrency ? ' <span class="' + (t.feeCurrency === 'BNB' ? 'text-success' : 'text-muted') + '" style="font-size:0.6rem" title="Fee paid in ' + t.feeCurrency + '">' + (t.feeCurrency === 'BNB' ? '✓' : t.feeCurrency) + '</span>' : '') : '-')}</td>
