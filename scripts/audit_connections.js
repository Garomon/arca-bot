...
async function auditConnections() {
    ...
    for (const pair of ["BTC", "SOL", "DOGE"]) {
        ...
        // Read spacing from state file (not hardcoded)
        const stateFile = "/root/arca-bot/data/sessions/VANTAGE01_" + pair + "USDT_state.json";
        let spacing = 0.01; // Default only if file doesn't exist
        ...
        for (const t of trades) {
            ...
            if (bestMatch) {
                ...
                console.log("    Reason: No buy before " + sellDate + " within " + (tolerance * 100).toFixed(1) + "% tolerance");
                console.log("    Min buy price needed: $" + minBuyPrice.toFixed(6));
                console.log("    NET PROFIT:     $0.000000 (UNMATCHED)");
            }
            console.log("\n");
        }
